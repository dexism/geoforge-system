...
- ブロック内のヘックス座標は、`ee-nn`で表し、南西角を原点（`00-00`）として東、北へ増加する。
- ワールド座標として扱う場合、`map_50_73.json`ブロックの`12-08`ヘックスを`5012-7308`と表す。
...
- 河川と道路は、画面表示用にブロック内の各ヘックス内に追加する。
